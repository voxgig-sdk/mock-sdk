...
class GetCustomResourceItemByIdEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "get_custom_resource_item_by_id"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    GetCustomResourceItemByIdEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MockHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetCustomResourceItemById, Hash] the current GetCustomResourceItemById data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MockHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetCustomResourceItemById fields)
  def match_get
  ...
  #
  # @param reqmatch [GetCustomResourceItemByIdLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetCustomResourceItemById.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetCustomResourceItemById, Hash] the loaded GetCustomResourceItemById; raises MockError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MockHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

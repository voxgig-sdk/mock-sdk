...
class CouponDirectTest < Minitest::Test
  def test_direct_list_coupon
    setup = coupon_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-coupon", setup[:live] ? "live" : "unit")
    if _should_skip
...
def coupon_direct_setup(mockres)

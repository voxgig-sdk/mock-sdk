...
class CartDirectTest < Minitest::Test
  def test_direct_list_cart
    setup = cart_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-cart", setup[:live] ? "live" : "unit")
    if _should_skip

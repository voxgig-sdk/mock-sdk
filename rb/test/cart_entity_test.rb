...
class CartEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Cart(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Cart(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = cart_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "cart." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MOCK_TEST_CART_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    cart_ref01_data = nil
    if cart_ref01_data_raw.length > 0
      cart_ref01_data = Helpers.to_map(cart_ref01_data_raw[0][1])
    end

    # LIST
    cart_ref01_ent = client.Cart(nil)
    cart_ref01_match = {}
...
def cart_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "cart", "CartTestData.json")
  ...
  entid_env_raw = ENV["MOCK_TEST_CART_ENTID"]

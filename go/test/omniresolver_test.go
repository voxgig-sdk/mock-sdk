...
import (
	...
	omni "github.com/voxgig-sdk/mock-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MockError); is {
				out := map[string]any{
					"name":    "MockError",
...
func omniCtx(arg any, client *sdk.MockSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MockSDK); is {
			client = live

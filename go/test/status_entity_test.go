...
func TestStatusEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MOCK_TEST_STATUS_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		statusRef01MatchDt0 := map[string]any{
			"id": statusRef01Data["id"],
		}
		statusRef01DataDt0Loaded, err := statusRef01Ent.Load(statusRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		statusRef01DataDt0LoadResult := core.ToMapAny(entityData(statusRef01DataDt0Loaded))
		if statusRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if statusRef01DataDt0LoadResult["id"] != statusRef01Data["id"] {
...
func statusBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MOCK_TEST_STATUS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MOCK_TEST_STATUS_ENTID"])

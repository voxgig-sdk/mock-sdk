...
{
    private static function utility(): MockUtility
    {
        return new MockUtility();
    }
    ...
    private static function ctx(array $over = []): MockContext
    {
        ...
        $ctx->op = new MockOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof MockError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new MockOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = MockMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new MockOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = MockMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new MockOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = MockMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MockSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = MockMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx([
            'spec' => new MockSpec(['step' => 's']),
            'response' => new MockResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new MockResult([]),
        ]);
        [, $err] = MockMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new MockSpec(['step' => 's']),
            'response' => new MockResponse(self::resp(200, ['v' => 1])),
            'result' => new MockResult([]),
        ]);
        [, $err] = MockMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new MockSpec(['step' => 's']),
            'response' => new MockResponse(self::resp(200, ['v' => 2])),
            'result' => new MockResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        MockMakeResponse::call($ctx);
    ...
    {
        $preset = new MockResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new MockSpec([]),
            'response' => new MockResponse([]),
            'result' => new MockResult([]),
        ...
        [$got, $err] = MockMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new MockResult([])]);
        [, $err] = MockMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new MockSpec([]), 'result' => null]);
        [, $err] = MockMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MockSpec(['step' => 's']),
            'result' => new MockResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new MockOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MockMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new MockSpec(['step' => 's']),
            'result' => new MockResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new MockOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = MockMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new MockResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new MockSpec([]),
            'result' => new MockResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = MockMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new MockError('boom', 'boom');
        $utility->fetcher = function (MockContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MockSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = MockMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (MockContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (MockContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new MockSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = MockMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new MockResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new MockSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = MockMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = MockMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new MockResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, MockDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new MockResult(['ok' => false])]);
    ...
    {
        $ctx = self::ctx(['result' => new MockResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', MockMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new MockBaseFeature();
        $b = new MockBaseFeature();
        MockFeatureAdd::call($ctx, $a);
        MockFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): MockBaseFeature
    {
        $f = new MockBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): MockContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new MockSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new MockResponse(['headers' => null]),
            'result' => new MockResult([]),
        ]);
        MockResultHeaders::call($ctx);

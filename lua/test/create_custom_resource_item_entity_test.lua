...
describe("CreateCustomResourceItemEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:CreateCustomResourceItem(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = create_custom_resource_item_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "create_custom_resource_item." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MOCK_TEST_CREATE_CUSTOM_RESOURCE_ITEM_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local create_custom_resource_item_ref01_ent = client:CreateCustomResourceItem(nil)
    local create_custom_resource_item_ref01_data = helpers.to_map(vs.getprop(
...
function create_custom_resource_item_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/create_custom_resource_item/CreateCustomResourceItemTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read create_custom_resource_item test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("MOCK_TEST_CREATE_CUSTOM_RESOURCE_ITEM_ENTID")

...
describe("CartEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Cart(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Cart(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MOCK_TEST_CART_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local cart_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.cart")))
    local cart_ref01_data = nil
    if #cart_ref01_data_raw > 0 then
      cart_ref01_data = helpers.to_map(cart_ref01_data_raw[1][2])
    end
...
function cart_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MOCK_TEST_CART_ENTID")

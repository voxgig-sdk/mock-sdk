...
describe("DeleteCustomResourceItemEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:DeleteCustomResourceItem(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MOCK_TEST_DELETE_CUSTOM_RESOURCE_ITEM_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local delete_custom_resource_item_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.delete_custom_resource_item")))
    local delete_custom_resource_item_ref01_data = nil
    if #delete_custom_resource_item_ref01_data_raw > 0 then
...
function delete_custom_resource_item_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/delete_custom_resource_item/DeleteCustomResourceItemTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read delete_custom_resource_item test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("MOCK_TEST_DELETE_CUSTOM_RESOURCE_ITEM_ENTID")

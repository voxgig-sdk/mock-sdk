-- Mock SDK Cart entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CartEntity = {}
CartEntity.__index = CartEntity


function CartEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CartEntity)
  self._name = "cart"
  self._client = client
...
function CartEntity:get_name()
  return self._name
end


function CartEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CartEntity.new(self._client, opts)
end
...
function CartEntity:mark_deleted()
  self._deleted = true
end


function CartEntity:deleted()
  return true == self._deleted
end


function CartEntity:data_set(args)
  if args ~= nil then
...
function CartEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CartEntity:match_set(args)
  if args ~= nil then
...
function CartEntity:stream(action, args, callopts)
...
function CartEntity:_run_op(ctx, post_done)
  local utility = self._utility

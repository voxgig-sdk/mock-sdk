...
local CouponEntity = {}
CouponEntity.__index = CouponEntity


function CouponEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CouponEntity)
  self._name = "coupon"
  self._client = client
...
function CouponEntity:get_name()
  return self._name
end


function CouponEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CouponEntity.new(self._client, opts)
end
...
function CouponEntity:mark_deleted()
  self._deleted = true
end


function CouponEntity:deleted()
  return true == self._deleted
end


function CouponEntity:data_set(args)
  if args ~= nil then
...
function CouponEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CouponEntity:match_set(args)
  if args ~= nil then
...
function CouponEntity:match_get()
...
function CouponEntity:stream(action, args, callopts)
...
function CouponEntity:_run_op(ctx, post_done)
  local utility = self._utility

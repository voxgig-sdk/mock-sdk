...
type Args struct {
	Entity string         `json:"entity" jsonschema:"cart | coupon | create_custom_resource_item | delete_custom_resource_item | get_custom_resource | get_custom_resource_item_by_id | patch_custom_resource_item | product | status | update_custom_resource_item | user"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.MockSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "mock_list",
		Description: "List records from Mock. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "mock_load",
		Description: "Load a single record from Mock. " +
...
func runOp(client *sdk.MockSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.MockSDK, name string) (sdk.MockEntity, error) {
	switch strings.ToLower(name) {
	case "cart":
		return client.Cart(nil), nil
	case "coupon":
		return client.Coupon(nil), nil
	case "create_custom_resource_item":
		return client.CreateCustomResourceItem(nil), nil
	case "delete_custom_resource_item":
		return client.DeleteCustomResourceItem(nil), nil
	case "get_custom_resource":
		return client.GetCustomResource(nil), nil
	case "get_custom_resource_item_by_id":
		return client.GetCustomResourceItemById(nil), nil
	case "patch_custom_resource_item":
		return client.PatchCustomResourceItem(nil), nil
	case "product":
		return client.Product(nil), nil
	case "status":
		return client.Status(nil), nil
	case "update_custom_resource_item":
		return client.UpdateCustomResourceItem(nil), nil
	case "user":

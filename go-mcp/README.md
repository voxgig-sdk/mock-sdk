...
[MCP](https://modelcontextprotocol.io) server exposing the Mock SDK as
two agent tools — `mock_list` and `mock_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mock-mcp)
make build

# 2. Provide credentials via the environment
export MOCK_APIKEY=sk_live_xxx
...
```jsonc
// mock_list: first page of records
{ "entity": "cart" }
{ "entity": "cart", "query": { } }

// mock_load: one record by id
{ "entity": "get_custom_resource_item_by_id", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mock-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mock \
     -- "$PWD"/dist/*/mock-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mock_list` and `mock_load` tools now appear
   in new sessions. Ask the agent to *"list cart using mock"*
   and it calls `mock_list` with `{"entity":"cart"}`.
...
```sh
export MOCK_APIKEY=sk_live_xxx            # API key
export MOCK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `mock_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mock_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MOCK_APIKEY` | API key sent with every request. |
| `MOCK_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mock_load","arguments":{"entity":"get_custom_resource_item_by_id","query":{"id":1}}}}'
```

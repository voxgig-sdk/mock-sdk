...
boru-driven command-line client **and** interactive REPL for the Mock
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mock-cli)
make build
...
./mock-cli list cart
./mock-cli list coupon

# 5. Override the API base URL for a single call
MOCK_BASE=https://api.example.com ./mock-cli list cart
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mock-cli
   ```
...
Run with no arguments to open a REPL (prompt `mock>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cart`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mock-cli`. |
...
(`./mock-cli <expr>`) and interactively (the REPL), and expressions compose the

// Command Mock-cli is a CLI + REPL for the Mock SDK,
...
const entitiesHelp = "cart coupon create_custom_resource_item delete_custom_resource_item get_custom_resource get_custom_resource_item_by_id patch_custom_resource_item product status update_custom_resource_item user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MOCK_APIKEY (api key), MOCK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MOCK_APIKEY carries the API key and
	// MOCK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MOCK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MOCK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMockSDK(opts)

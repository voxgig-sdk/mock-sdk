...
from mock_sdk.mock_types import (
...
class DeleteCustomResourceItemEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeleteCustomResourceItemEntity(self._client, opts)
    ...
    def data_get(self) -> DeleteCustomResourceItem:
    ...
    def match_get(self) -> DeleteCustomResourceItem:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DeleteCustomResourceItem:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DeleteCustomResourceItem().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}

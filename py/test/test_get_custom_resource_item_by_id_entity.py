...
class TestGetCustomResourceItemByIdEntity:

    def test_should_create_instance(self):
        testsdk = MockSDK.test(None, None)
        ent = testsdk.GetCustomResourceItemById(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _get_custom_resource_item_by_id_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "get_custom_resource_item_by_id." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MOCK_TEST_GET_CUSTOM_RESOURCE_ITEM_BY_ID_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_custom_resource_item_by_id_ref01_data = None
        if len(get_custom_resource_item_by_id_ref01_data_raw) > 0:
...
def _get_custom_resource_item_by_id_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/get_custom_resource_item_by_id/GetCustomResourceItemByIdTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MOCK_TEST_GET_CUSTOM_RESOURCE_ITEM_BY_ID_ENTID"))

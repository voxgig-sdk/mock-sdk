...
class TestCartDirect:

    def test_should_direct_list_cart(self):
        setup = _cart_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-cart", "live" if setup["live"] else "unit")
        if _skip:
...
def _cart_direct_setup(mockres):

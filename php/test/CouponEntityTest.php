...
class CouponEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = MockSDK::test($seed, null);
        $seen = iterator_to_array($base->Coupon(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = MockConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = MockSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Coupon(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = coupon_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "coupon." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MOCK_TEST_COUPON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $coupon_ref01_data = null;
        if (count($coupon_ref01_data_raw) > 0) {
...
function coupon_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/coupon/CouponTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["coupon01", "coupon02", "coupon03"] as $k) {
    ...
    $entid_env_raw = getenv("MOCK_TEST_COUPON_ENTID");

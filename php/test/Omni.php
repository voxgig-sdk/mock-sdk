...
//    only - a typed MockContext there would make every ctx
...
final class MockOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MockContext($clean, null);
    ...
    public static function err_from_map($m): ?MockError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MockError(is_string($code) ? $code : '', $msg);
    }

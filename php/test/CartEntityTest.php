...
class CartEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Cart(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Cart(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MOCK_TEST_CART_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $cart_ref01_data = null;
        if (count($cart_ref01_data_raw) > 0) {
...
function cart_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MOCK_TEST_CART_ENTID");

...
class FtCtrl extends MockControl
{
...
class FtClient
{
    ...
    private MockUtility $utility;

    public function __construct(MockUtility $utility, array $options)
    {
    ...
    public function get_utility(): MockUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MockContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MockContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MockConfig::make_feature($name);
    ...
    private static function buildUrl(MockSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MockError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MockResponse($fetched);
        }
    ...
    private function fail(MockContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MockContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

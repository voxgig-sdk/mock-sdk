...
class UpdateCustomResourceItemEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MockSDK::test(null, null);
        $ent = $testsdk->UpdateCustomResourceItem(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = update_custom_resource_item_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "update_custom_resource_item." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MOCK_TEST_UPDATE_CUSTOM_RESOURCE_ITEM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $update_custom_resource_item_ref01_data = null;
        if (count($update_custom_resource_item_ref01_data_raw) > 0) {
        ...
        [$update_custom_resource_item_ref01_resdata_up0_result, $err] = $update_custom_resource_item_ref01_ent->update($update_custom_resource_item_ref01_data_up0_up, null);
        $this->assertNull($err);
...
function update_custom_resource_item_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/update_custom_resource_item/UpdateCustomResourceItemTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["update_custom_resource_item01", "update_custom_resource_item02", "update_custom_resource_item03", "resource01"] as $k) {
    ...
    $entid_env_raw = getenv("MOCK_TEST_UPDATE_CUSTOM_RESOURCE_ITEM_ENTID");

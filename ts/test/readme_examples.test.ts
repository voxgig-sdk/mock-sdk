...
//     (\`new MockSDK(...)\` / \`MockSDK.test(...)\` become
//     \`MockSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cart":{"test01":{"id":"test01"}},"coupon":{"test01":{"id":"test01"}},"create_custom_resource_item":{"test01":{"id":"test01"}},"delete_custom_resource_item":{"test01":{"id":"test01"}},"get_custom_resource":{"test01":{"id":"test01"}},"get_custom_resource_item_by_id":{"test01":{"id":"test01"}},"patch_custom_resource_item":{"test01":{"id":"test01"}},"product":{"test01":{"id":"test01"}},"status":{"test01":{"id":"test01"}},"update_custom_resource_item":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MockSDK(...)\` and any
// \`MockSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MockSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MockSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
describe('GetCustomResourceItemByIdDirect', async () => {
  ...
  test('direct-load-get_custom_resource_item_by_id', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_custom_resource_item_by_id', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id01","resource01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MOCK_TEST_GET_CUSTOM_RESOURCE_ITEM_BY_ID_ENTID']

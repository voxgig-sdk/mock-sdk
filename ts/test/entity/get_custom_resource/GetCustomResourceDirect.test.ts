...
describe('GetCustomResourceDirect', async () => {
  ...
  test('direct-list-get_custom_resource', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_custom_resource', setup.live)) return
    if (skipIfMissingIds(t, setup, ["get_custom_resource01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MOCK_TEST_GET_CUSTOM_RESOURCE_ENTID']
